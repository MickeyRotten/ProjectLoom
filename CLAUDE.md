...
Post-MVP also: **places** (`places.ts`, `generatePlace.ts`, `GameState.places` +
`GameState.area`, Menu → Places) — `location` was one name, the most specific one
(`simplifyLocation` enforces it), so the narrator knew it was in the "Damp
Cellar" and re-improvised the tavern, the town and everyone in them every turn.
A `Place` is the AREA that room sits inside, authored once by a side call the
first time the player walks in and then read back as authority. **Three kinds**
— `steading` · `dungeon` · `wild` — differing only in *which tag slots exist*
(`PLACE_KINDS`, one table driving the generator prompt, the editor and the
prompt block, so the conditional rule can't turn into three drifting `if`
ladders); storage is always `tags: { slot, value }[]` plus rooms, rumours and
keywords. Options are **suggestions, not a whitelist** (a value outside them is
kept verbatim; only the slot *key* is validated, and a slot the kind lacks is
dropped). **Rooms are common ⟂ unique** — common is a palette the narrator may
reuse, unique exists exactly once — with nothing tracking visits: the label is
context, not a map. **Rumours are believed, not true**, and the block says so.
The narrator emits `"area"` beside `"location"` (with a compound location's
discarded head as a fallback, `locationParent`/`resolveArea`); an unknown name
appends a **stub synchronously before `captureReversal`** and fills it
asynchronously, the `journal.ts` race fix, so undo un-discovers a place and
`fillPlace` no-ops on a missing id. Re-entry costs nothing. The call is the one
generate* flow that **reads the beats** — what the player walked into was just
described in the prose. `trade` tags name neighbours, each becoming a stub of
its own (`addNeighbourStubs`) — free world-building, no extra call. Injection is
two-sited: the current area in tier 5 in full, other places keyword-gated into
tier 2 trimmed to name/kind/description. **No place delta channel** — the
...
The steading tag vocabulary in `src/lib/places.ts` is from **Dungeon World** by
