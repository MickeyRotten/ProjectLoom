...
lands them on the faces the turn actually rolled — on a **tilted surface**
(`SCENE_TILT`, one rotation shared by every die, so they rest parallel like dice
on a table rather than square to the glass) — shows the arithmetic and the band
on an opaque plate, and fades out (~2.4s, tap to skip). Staged in `sendTurn` *before* the model call, so it
